...
main( int argc , char * argv[] )
{
   ...
   * - les cellules sont choisies au hasard et évoluent sans tenir compte de leur <i>"historique"</i> 
   * - on re-génère l'automate entier à chaque cellule choisie. Il n'y a qu'une seule cellule qui évolue à chaque génération.
   */
  ...
  for( n=1 ; n<=nb_generations ; n++ )
      ...
      if( (noerr = automate_cellule_evoluer( automate , automate_get( automate , i , j ) , &regles ) ) )
	{
      ...
      sprintf( mess , "ASYNC %d" , n ) ;
      ecran_message_afficher( ecran , mess ) ;
      usleep(100000);
      
      /*! <li> Passage prochaine génération (en réalité uniquement pour la cellule choisie) */
