 ...
 * <P>
 * Il est possible d'ajouter et retirer des articles, changer les quantit�s des
 * articles d�j� pr�sents, vider le panier de tous les articles, imprimer un
 * ticket tenant compte ou non d'une remise param�trable � partir d'un seuil
 * param�trable
 * 
...
public class Panier {
	...
	/**
	 * Permet d'ajouter un nombre d'exemplaire donn� d'un article pr�sent ou non
	 * dans le panier
	 * 
	 * @param art
	...
	public void ajouterArticle(Article art, int qte)
		...
		if (!articleEstDansPanier(art.lireReference())) {
				...
				if (qte < 10) {
					if (prixActuel + supplement <= 1000) {
						...
						throw new MontantTropEleveException(
								"L'ajout de " + qte + " " + art.lireReference() + " au prix unitaire HT de "
										+ art.lirePrixHT() + " � un panier de " + prixActuel + " HT d�passe 1000.");
				} else {
					throw new QuantiteArticleTropGrandeException("Le nombre d'exemplaires (" + qte
							+ ") de l'article de r�f�rence " + art.lireReference() + " d�passe 10.");
				}
			} else {
				throw new TropDeReferencesException("Article de r�f�rence " + art.lireReference()
			...
			if (qte + deja < 10) {
				...
				throw new QuantiteArticleTropGrandeException("L'article de r�f�rence " + ref + " est pr�sent en " + deja
						+ " exemplaires. Il ne peut �tre ajout� en " + qte + " exemplaires dans le panier.");
			}
		}
	}
	 ...
	 * @return vrai si l'article consid�r� est pr�sent dans le panier, faux dans
	 *         le cas contraire
	 */
	...
	public void modifierQuantiteArticle(String ref, int qte)
			throws ArticleInexistantException, QuantiteArticleTropGrandeException, MontantTropEleveException {
		final LignePanier lp = lireLignePanier(ref);
		if (lp != null) {
			if (qte < 10) {
				final double prixActuel = prixHTPanier();
				...
				if (prixActuel + supplement <= 1000) {
					...
					throw new MontantTropEleveException("Faire passer le nombre d'exemplaires de " + art.lireReference()
							+ " � " + qte + " conduit � un panier de prix HT sup�rieur � 1000.");
			} else
				throw new QuantiteArticleTropGrandeException("Le nombre d'exemplaires � modifier (" + qte
						+ ") de l'article de r�f�rence " + ref + " d�passe 10.");
		} else
			throw new ArticleInexistantException("Reference " + ref + " inexistante dans le panier");
	}

	/**
	 * Renvoie le nombre d'exemplaires dans le panier d'un article identifi� par
	 * sa r�f�rence
	 * 
	 * @param ref
	 *            est la r�f�rence de l'article recherch�
	 ...
	 * consid�r� n'est pas pr�sent, une exception ArticleInexistantException est
	 * lev�e
	 * 
	 * @param ref
	 *            est la r�f�rence de l'article consid�r�
	 ...
	 * 
	 * @param montantSeuil
	 *            est le montant � partir duquel une remise est d�duite du prix
	 *            du panier
	 */
	...
	public String lireTicket() {
		...
		for (LignePanier lp : contenuPanier.values()) {
			...
			String prix = OutilsChaine.formatMonnaie.format(lp.lireArt().lirePrixHT());
		...
		String pct = OutilsChaine.formatMonnaie.format(this.lirePourcentageRemise());
		remise = OutilsChaine.formater(remise, 24 - pct.length(), 0, ' ');
		ticket += "| REMISE " + pct + "%" + remise + " |\n";
		ticket += "------------------------------------" + "\n";
		String TVA = OutilsChaine.formatMonnaie.format(this.prixTTCPanier() - this.prixHTPanier());
		TVA = OutilsChaine.formater(TVA, 29, 0, ' ');
		ticket += "| TVA" + TVA + " |\n";
		ticket += "------------------------------------" + "\n";
		String NET = OutilsChaine.formatMonnaie.format(this.prixTTCPanier() - this.montantRemise());
